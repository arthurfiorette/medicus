...
export const medicusPlugin = fp<FastifyMedicsPluginOptions>(
  async (fastify, { route, debug = false, hide, ...medicusOptions }) => {
    ...
    if (fastify.hasPlugin('@fastify/under-pressure')) {
      fastify.medicus.addChecker(async function pressure() {
        return {
          //@ts-expect-error - only if @fastify/under-pressure is installed
    ...
    fastify.route({
      ...
      async handler(request, reply) {
        const result = await this.medicus.performCheck();

        reply.status(healthStatusToHttpStatus(result.status));

        if (typeof debug === 'boolean' ? debug : await debug(request)) {
          return result;
        }
